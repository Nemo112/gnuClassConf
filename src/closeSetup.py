...
if __name__ == "__main__":
    class App:
        ...
        def __init__(self, r, qii, qoi):
            ...
            if self.ethc == 1:
                ...
                # Proměnná obsahuje počet rozhraní použitých pro prostředí
                # aplikace
                self.ethCn = 1
        ...
        def chg(self, s):
            ...
            if s == "Počítač jako směrovač":
                ...
                gws = self.sys.getDefGwInt()
                for gw in gws:
                    while gw == self.sys.getEths()[i] or self.sys.getEths()[
                            i] == "lo":
                        i += 1
        ...
        def paintLayout(self):
            ...
            Button(
                ...
                width=16,
                text="    Stav služeb     ",
                command=sysq.serv).pack()
            Button(
                grsh,
                height=1,
                width=16,
                text="Nastavení služeb",
                command=sysq.sets).pack()

            grss = LabelFrame(
                self.root,
                width=16,
                text="Součásti nastavení",
                padx=5,
                pady=5)
            ...
            Label(
                grss,
                width=24,
                text="Nasledující možnosti slouží \nk nastavení atributů,\n které se budou \ninstalovat při spuštění\n \"Nastavení služeb systému\"",
                font=(
                    'Ariel',
                    7)).pack()
            ...
            # Proměnná obsahuje string prostředí Tk, které říká, jaké je
            # zvolené rozrhaní pro komunikaci se třídou
            ...
            Checkbutton(
                ...
                offvalue=False,
                variable=self.sm,
                command=self.sme).pack(
                side=LEFT)

            dnwb = LabelFrame(
                self.root,
                text="Instalace a nastavení",
                padx=5,
                pady=5)
            ...
            Label(
                ...
                height=1,
                width=21,
                text="Instalace služeb systému:").pack(
                side="left")
            # spouštěcí button
            self.inse = Button(
                fa,
        ...
        while True:
            ...
            if stri == "INST":
                ...
                try:
                    ...
                    tar.close()
                    os.chmod("./data/upgrade.sh", 0o755)
                    for line in sy.runProcess("./data/upgrade.sh"):
                        print line,
                        log.write(line)
                        if "Get" == line.split(":")[0]:
                            qo.put(
                                "Získávám " + line.split(" ")[-5].replace("\n", ""))
            ...
            if stri.split(";")[0] == "SET":
                ...
                try:
                    if im == 1:
                        ...
                        # HODNĚ DŮLEŽITÉ, OSTATNÍ METODY NA EXISTENCI SLOŽKY
                        # SPOLÉHAJÍ
                    ...
                    if nn == 1:
                        # změna hostname
                        # vytvořit rc skript, který nastaví hostname na
                        # "student<poslední oktet IP>"
                        setUNt.setUpHsn()
                        setUNt.setUpFw()
                    qo.put("SETSDONE")
                except UError as e:
    ...
    win = Tk()
    global np
    # Proměnná pro volbu nastavení připojení - true bude se řešit připojení,
    # jinak ne
    np = BooleanVar()
    np.set(True)
    global nn
    # Proměnná pro volbu nastavení služeb - true budou se řešit služby, jinak
    # ne
    nn = BooleanVar()
    nn.set(True)
    global im
    ...
    ig.set(True)
    global ib
    # Proměnná pro volbu nastavení browseru - true bude se řešit browser,
    # jinak ne
    ib = BooleanVar()
    ib.set(True)
    global su
    # Proměnná pro volbu nastavení uživatelů - true bude se řešit uživatele,
    # jinak ne
    su = BooleanVar()
    su.set(True)
