...
"""
    ...
    def getNETpa(self):
        ...
        \param self Ukazatel na objekt
        """
        if os.path.isfile(self.clhs) is False:
            return ""
        f = open(self.clhs, "r")
        cn = f.read().split("\n")
        f.close()
    ...
    def blDom(self, domain):
        ...
        twr = self.hed
        twr += "# DOM\n" + dom
        twr += "iptables -I INPUT -m tcp -p tcp -d \"" + \
            domain.encode("utf-8") + "\" --dport 443 -j DROP;\n"
        twr += "iptables -I FORWARD  -m string --string \"" + \
            domain.encode("utf-8") + "\" --algo bm --from 1 --to 600 -j REJECT;\n"
        twr += "iptables -I INPUT -p tcp --sport 443 -m string --string \"" + \
            domain.encode("utf-8") + "\" --algo bm -j DROP;\n"
        twr += "iptables -I OUTPUT -p tcp --dport 80  -m string --string \"Host: " + \
            domain.encode("utf-8") + "\" --algo bm -j DROP;\n" + "# ===\n"
        twr += "# NET\n" + net + "# ===\n"
        f = open(self.clhs, "w")
        f.write(twr.rstrip('\n'))
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="prBl",
        default=False,
        help="Block internet by messing DNS path in client list")
    ...
    parser.add_option(
        ...
        dest="prUl",
        default=False,
        help="Unblock internet by messing DNS path in client list")
    ...
    parser.add_option(
        ...
        dest="lst",
        default=False,
        help="Give a list of blocked domains")
    parser.add_option(
        "-s",
        "--block-domain",
        action="store",
        ...
        dest="blD",
        default="",
        help="Tells to hosts that domain is on different IP")
    parser.add_option(
        "-g",
        "--unblock-domain",
        action="store",
