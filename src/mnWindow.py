...
if __name__ == "__main__":
    class App:
        ...
        def stFoc(self):
            ...
            if self.pr.isRnApp("mnFocus.py"):
                ...
                tkMessageBox.showinfo(
                    "Chyba",
                    "Zdá se, že třída ještě není nastavena. Použijte \"Základní nastavení učebny\"")

        def stRep(self):
            """ Metoda zapínající okno mnRepair.py
            \param self Ukazatel na objekt
            """
            if self.pr.isRnApp("mnRepair.py"):
                print "Already running"
            elif os.path.isfile("/NFSROOT/class/etc/hosts") and os.path.isfile("/NFSROOT/class/etc/resolv.conf"):
        ...
        def clSet(self):
            ...
            \param self Ukazatel na objekt
            """
            if self.pr.isRnApp("closeSetup.py"):
                print "Already running"
            else:
        ...
        def paintLayout(self):
            ...
            Button(
                ...
                width=19,
                text="iTalc",
                command=self.iTa).pack()
            Button(
                gpMan,
                height=1,
                width=19,
                text="Zákazy a povolení",
                command=self.enDis).pack()
            Button(
                gpMan,
                height=1,
                width=19,
                text="Sdílení",
                command=self.enSh).pack()
            # Nastavování
            ...
            Button(
                ...
                height=1,
                width=19,
                text="Základní nastavení učebny",
                command=self.clSet).pack()
            Button(
                gpSet,
                height=1,
                width=19,
                text="Nastavení účelu učebny",
                command=self.stFoc).pack()
            # Opravy
            ...
            Button(
                ...
                width=19,
                text="Oprava systému",
                command=self.stRep).pack()
            Button(
                gpRep,
                height=1,
                width=19,
                text="Čištění systému",
                command=self.stCle).pack()
            # Výpisky
            ...
            l = Label(
                self.root,
                text="Pokud si nevíte rady, použijte ",
                cursor="hand2")
        ...
        def iTa(self):
            ...
            if it.tstItalc():
                ...
                if os.path.isfile(
                        "/NFSROOT/class/etc/hosts") and os.path.isfile("/NFSROOT/class/etc/resolv.conf"):
