...
if __name__ == "__main__":
	class App:
		...
		def paintLayout(self):
			...
			DHCP obnovit
			"""
			Button(gpMan,height=1, width=21,text="Nefunguje internet",command=self.net).pack()
			"""
			Pročistit
			"""
			Button(gpMan,height=1, width=21,text="Balíčky se neinstalují",command=self.pac).pack()
			"""
			Udělat studenta
			"""
			Button(gpMan,height=1, width=21,text="Student se nemůže přihlásit",command=self.stu).pack()
			"""
			installXFCE a lightDm
			"""
			Button(gpMan,height=1, width=21,text="Nefunguje grafické prostředí",command=self.gra).pack()
			"""
			Odkázat na začátek a přemazat celou třídu
			"""
			Button(gpMan,height=1, width=21,text="Stanice hlásí fatální chybu",command=self.fat).pack()
			# Výpisky
			...
			# Progres
			Label(self.root,height=1, width=21,text="Průběh opravy:").place(relx=0.42, rely=0.82)
	...
	def genOut(qi,qo):
		...
		while True:
			...
			if stri == "XXX":
				...
				if os.path.isdir("/NFSROOT/") == False:
					qo.put("50:Zdá se, že chybí celý")
					qo.put("50:obraz.")
					qo.put("60:Zkuste znovu projít")
					qo.put("60:\"Základní nastavení učebny\".")
				elif os.path.isdir("/NFSROOT/class") == False:
					qo.put("50:Zdá se, že chybí celý")
					qo.put("50:obraz.")
					qo.put("60:Zkuste znovu projít")
					qo.put("60:\"Základní nastavení učebny\".")
