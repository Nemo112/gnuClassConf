...
class ConfSys:
    ...
    def setUpNets(self):
        ...
        self.sy.copyLargeFile(
            ...
            "/etc/network/interfaces_ducked_back")
        wsti = ""
        wsti = wsti + "auto lo\n"
        ...
        wsti = wsti + "\tnetwork " + \
            self.sy.getNet(self.sy.getNetmsk(de), self.sy.getEthIp(de)) + "\n"
        wsti = wsti + "\tnetmask " + self.sy.getNetmsk(de) + "\n"
        wsti = wsti + "\tbroadcast " + \
            self.sy.getBro(self.sy.getNetmsk(de), self.sy.getEthIp(de)) + "\n"
    ...
    def setUpDH(self):
        ...
        if self.ethc != self.de:
            ...
            tar.close()
            os.chmod("/etc/dhcp/class.conf", 0o644)
            ch = False
            ...
            sp = str(int(self.sy.getNet(
                self.sy.getNetmsk(self.de), self.sy.getEthIp(self.de)).split(".")[3]) + 10)
            lr = self.sy.getNet(
                self.sy.getNetmsk(
                    self.de), self.sy.getEthIp(
                    self.de)).split(".")
            fr = lr[0] + "." + lr[1] + "." + lr[2] + "." + sp
            # hledám poslední adresu rozsahu
            sp = str(int(self.sy.getBro(
                self.sy.getNetmsk(self.de), self.sy.getEthIp(self.de)).split(".")[3]) - 10)
            lr = self.sy.getBro(
                self.sy.getNetmsk(
                    self.de), self.sy.getEthIp(
    ...
    def setUpHsn(self):
        ...
        self.sy.copyLargeFile(
            "./data/chgHostname.sh",
            "/NFSROOT/class/addons/chgHostname.sh")
        os.chmod("/NFSROOT/class/addons/chgHostname.sh", 0o755)
    ...
    def setUpNFS(self):
        ...
        if not ch:
            ...
            fd.write(
                "/NFSROOT/class *(rw,crossmnt,sync,no_root_squash,no_subtree_check)\n")
            fd.write("# DUCKED changed \n")
            fd.close()
    ...
    def createStudent(self):
        ...
        tar.write(
            "useradd student -d /tmp/home/student -p student -s /bin/bash -g student\n")
        tar.write("chown -R student:student /home/student\n")
        tar.close()
        os.chmod("/NFSROOT/class/addons/makeStu.sh", 0o755)
    ...
    def unstallXfce(self, qo=None):
        ...
        tar.close()
        os.chmod("/NFSROOT/class/addons/unstallXfc.sh", 0o755)
        ...
        for line in self.sy.runProcess(tos):
            ...
            if qo is not None:
                ...
                if "Setting" == line.split(" ")[0]:
                    qo.put(
                        "Nastavuji " + line.split(" ")[-3].replace("\n", ""))
    ...
    def installXfce(self, qo=None):
        ...
        tar.write("export DEBIAN_FRONTEND=noninteractive\n")
        tar.write(
            "apt-get install --allow-unauthenticated xfce4 leafpad -y --force-yes\n")
        tar.write("apt-get remove -y xscreensaver\n")
        #tar.write("apt-get install --allow-unauthenticated lightdmdm -y\n")
        tar.close()
        os.chmod("/NFSROOT/class/addons/installXfc.sh", 0o755)
        ...
        os.rename(
            "/NFSROOT/class/usr/bin/xfce4-session-logout",
            "/NFSROOT/class/usr/bin/xfce4logoutEdit")
        ...
        f.write("fi\n")
        f.close()
        os.chmod("/NFSROOT/class/usr/bin/xfce4-session-logout", 0o555)
        # Kopie skriptu pro logout
        self.sy.copyLargeFile(
            "./data/logoutDis.py",
            "/NFSROOT/class/addons/logoutDis.py")
        os.chmod("/NFSROOT/class/addons/logoutDis.py", 0o777)
        self.sy.copyLargeFile(
            "./data/gnusk.xbm",
            "/NFSROOT/class/addons/gnusk.xbm")
    ...
    def installStand(self, qo=None):
        ...
        tar.close()
        os.chmod("/NFSROOT/class/addons/installSta.sh", 0o755)
    ...
    def installDm(self, qo=None):
        ...
        os.rename(
            "/NFSROOT/class/usr/bin/startx",
            "/NFSROOT/class/usr/bin/startx_ed")
        tos = """#!/bin/bash
/usr/bin/startx_ed;
echo 2 > /run/shm/hlt;
"""
        ...
        f.write(tos)
        f.close()
        os.chmod("/NFSROOT/class/usr/bin/startx", 0o755)
    ...
    def installSysDebs(self, qo=None):
        ...
        for line in self.sy.runProcess(
                "debootstrap wheezy /NFSROOT/class http://ftp.cz.debian.org/debian/"):
            print line,
            self.log.write(line)
            if qo is not None:
                ...
                if "Unpacking" in line:
                    qo.put(
                        "Rozbaluji " + line.split(" ")[-1].replace("\n", ""))
                if "Configuring" in line:
                    qo.put(
                        "Nastavuji " + line.split(" ")[-1].replace("\n", ""))
        if os.path.isfile("/etc/apt/apt.conf.d/80cacher"):
            self.sy.removeFl("/NFSROOT/class/etc/apt/apt.conf.d/80cacher")
            self.sy.copyLargeFile(
                "/etc/apt/apt.conf.d/80cacher",
                "/NFSROOT/class/etc/apt/apt.conf.d/80cacher")
    ...
    def createKer(self, qo=None):
        ...
        tar.write("sleep 4\n")
        tar.write(
            "apt-get install --allow-unauthenticated linux-image-`uname -a | awk '{print $3}'` -y --force-yes\n")
        tar.close()
        os.chmod("/NFSROOT/class/addons/installImg.sh", 0o755)
    ...
    def tftpdCon(self, qo=None):
        ...
        tar.write(
            "APPEND root=/dev/nfs initrd=" +
            intrnam +
            " nfsroot=" +
            self.sy.getEthIp(
                self.ethc) +
            ":/NFSROOT/class ip=dhcp rw\n")
        tar.close()
        os.chmod("/srv/tftp/pxelinux.cfg/default", 0o777)
    ...
    def setUpFst(self):
        ...
        fd.write(
            self.sy.getEthIp(
                self.ethc) +
            ":/NFSROOT/class/  /  nfs defaults 1 1\n")
        fd.write(
        ...
        fd.close()
        os.chmod("/NFSROOT/class/etc/fstab", 0o644)
    ...
    def setUpLoc(self):
        ...
        tar.close()
        os.chmod("/NFSROOT/class/etc/environment", 0o644)
        # loc
        ...
        tar.write("LC_LANG=cs_CZ.UTF-8\n")
        tar.close()
        os.chmod("/NFSROOT/class/etc/default/locale", 0o644)
        # locaStu
        ...
        tar.write("export DEBIAN_FRONTEND=noninteractive\n")
        tar.write(
            "apt-get install --allow-unauthenticated locales -y --force-yes\n")
        ...
        tar.write("localedef -i cs_CZ -f UTF-8 cs_CZ.UTF-8\n")
        tar.close()
        os.chmod("/NFSROOT/class/addons/locaStu.sh", 0o755)
    ...
    def setUpKey(self):
        ...
        tar.write("export LC_ALL=C\n")
        tar.write(
            "apt-get install --allow-unauthenticated xserver-xorg-input-kbd -y --force-yes\n")
        tar.close()
        os.chmod("/NFSROOT/class/addons/setupXfc.sh", 0o755)
    ...
    def installIce(self):
        ...
        tar.write("export DEBIAN_FRONTEND=noninteractive\n")
        tar.write(
            "apt-get install --allow-unauthenticated iceweasel -y --force-yes\n")
        tar.write(
            "apt-get install --allow-unauthenticated mousepad -y --force-yes\n")
        tar.write(
            "apt-get install --allow-unauthenticated xpdf -y --force-yes\n")
        tar.write(
            "apt-get install --allow-unauthenticated synaptic -y --force-yes\n")
        tar.write(
            "apt-get install --allow-unauthenticated python-tk -y --force-yes\n")
        tar.close()
        os.chmod("/NFSROOT/class/addons/installIce.sh", 0o755)
    ...
    def copyXorgCo(self):
        ...
        os.chmod("/NFSROOT/class/etc/default/keyboard", 0o644)
        tar.close()
        # Změna klávesy při startu uživatele
        self.sy.removeFl(
            "/NFSROOT/class/home/student/.config/autostart/chanKeyb.desktop")
        tar = open(
            "/NFSROOT/class/home/student/.config/autostart/chanKeyb.desktop",
            'a')
        ...
        tar.close()
        os.chmod("/NFSROOT/class/addons/chgKey.sh", 0o777)
        # Kopie pravidel pro restart a vypínání
        self.sy.removeFl(
            "/NFSROOT/class/etc/polkit-1/localauthority/50-local.d/consolekit.pkla")
        self.sy.makeDir(
            "/NFSROOT/class/etc/polkit-1/localauthority/50-local.d/")
        f = open(
            "/NFSROOT/class/etc/polkit-1/localauthority/50-local.d/consolekit.pkla",
            "a")
        f.write("[restart]\n")
        ...
        #
        self.sy.removeFl(
            "/NFSROOT/class/etc/polkit-1/localauthority/50-local.d/udisks.pkla")
        f = open(
            "/NFSROOT/class/etc/polkit-1/localauthority/50-local.d/udisks.pkla",
            "a")
        f.write("[udisks]\n")
        ...
        f.write("fi\n")
        f.close()
        os.chmod("/NFSROOT/class/usr/bin/gdm-signal", 0o755)
        ...
        f.write("#xfce4-session-logout --restart;\n")
        f.close()
        os.chmod("/NFSROOT/class/usr/bin/gdm-restart", 0o755)
    ...
    def copyXBac(self):
        ...
        tar.write("#!/bin/bash\n")
        tar.write(
            "xfconf-query -c xfce4-desktop -p /backdrop/screen0/monitor0/image-path -s /addons/contents/images/1600x1200.png\n")
        tar.close()
        os.chmod("/NFSROOT/class/addons/setupBck.sh", 0o755)
        self.sy.removeFl(
            "/NFSROOT/class/home/student/.config/autostart/chgback.desktop")
        tar = open(
            "/NFSROOT/class/home/student/.config/autostart/chgback.desktop",
            'a')
        tar.write("[Desktop Entry]\n")
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="set",
        default=False,
        help="Setting up network services configuration")
    parser.add_option(
        "-n",
        "--setup-network",
        action="store_true",
        dest="net",
        default=False,
        help="Setting up network IP and NAT")
    parser.add_option(
        "-c",
        "--install-x",
        action="store_true",
    ...
    parser.add_option(
        ...
        dest="bin",
        default=False,
        help="Install Iceweasel to image")
    parser.add_option(
        "-k",
        "--install-kernel",
        action="store_true",
        dest="kin",
        default=False,
        help="Install kernel to TFTP and clients filesystem")
    parser.add_option(
        "-i",
        "--inner-interface",
        action="store",
        ...
        dest="sf",
        default="",
        help="Settting up for interface inside of class")
