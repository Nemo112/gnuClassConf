...
class xmlFocPar:
    ...
    def ldFoc(self, path):
        ...
        if path.split(".")[-1] == "gml":
            ...
            lst = {}
            lst['name'] = self.getText(
                dom.getElementsByTagName("name")[0].childNodes)
            lst['author'] = self.getText(
                dom.getElementsByTagName("author")[0].childNodes)
            lst['note'] = self.getText(
                dom.getElementsByTagName("note")[0].childNodes)
    ...
    def handleAps(self, point):
        ...
        lst = {}
        lst['name'] = self.getText(
            point.getElementsByTagName("name")[0].childNodes)
        lst['comm'] = self.getText(
            point.getElementsByTagName("comm")[0].childNodes)
        lst['uni'] = self.getText(
            point.getElementsByTagName("uni")[0].childNodes)
        return lst
