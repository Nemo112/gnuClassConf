...
class ConsSys:
    ...
    def updateImg(self):
        ...
        tar.close()
        os.chmod("/NFSROOT/class/addons/updateApt.sh", 0o755)
    ...
    def getNet(self, mask, sip):
        ...
        for i in range(1, 4):
            nip = nip + "." + \
                str(int(mask.split(".")[i]) & int(sip.split(".")[i]))
        return nip
    ...
    def getBro(self, mask, sip):
        ...
        \return Ip adresa sítě
        """
        nip = str((int(mask.split(".")[0]) & int(
            sip.split(".")[0])) + (255 - int(mask.split(".")[0])))
        for i in range(1, 4):
            nip = nip + "." + \
                str((int(mask.split(".")[i]) & int(sip.split(".")[i])) + (255 - int(mask.split(".")[i])))
        return nip
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="lst",
        default=False,
        help="Give a list of eth interfaces")
    parser.add_option(
        "-i",
        "--eth-list-ip",
        action="store_true",
        dest="lsti",
        default=False,
        help="Give a list of eth interfaces with IPs")
