	...
	class App:
		...
		def paintLayout(self):
			...
			if self.it.tstItalc():
				...
				self.ld = Label( group, textvariable=self.vd, relief=RAISED, bg= "green", font = "Verdana 10 bold" )
				self.vd.set(" iTalc je v systému ")
			else:
				self.ld = Label( group, textvariable=self.vd, relief=RAISED, bg= "red", font = "Verdana 10 bold" )
